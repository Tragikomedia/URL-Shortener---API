...
describe('extractData', () => {
  it('Given an array of links, should return an array of objects containing targetURL and shortURI', async () => {
    ...
    expect(
      linksData.find(
        (link) =>
          link.targetURL === link1.targetURL && link.shortURI === link1.shortURI
      )
    );
    expect(
      linksData.find(
        (link) =>
          link.targetURL === link2.targetURL && link.shortURI === link2.shortURI
      )
    );
    expect(
      linksData.find(
        (link) =>
          link.targetURL === link3.targetURL && link.shortURI === link3.shortURI
      )
    );
  });
});
